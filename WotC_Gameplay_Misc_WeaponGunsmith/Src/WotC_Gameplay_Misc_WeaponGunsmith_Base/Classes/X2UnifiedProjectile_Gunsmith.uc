class X2UnifiedProjectile_Gunsmith extends X2UnifiedProjectile dependson(XComWeapon_Gunsmith);

var XComWeapon_Gunsmith		WeaponGSArc;
var XGWeapon_Platform_AR	WeaponVisualizer;
								...
								XComWeapon InSourceWeapon)
{
	...
	GunsmithState = XCGS_WeaponGunsmith(ItemState.FindComponentObject(class'XCGS_WeaponGunsmith'));
...
//	Projectiles[Index].TargetAttachActor.AttachComponent( PSComponent );
	...
	{
		//`log("Playing Weapon Additive Animation", , 'WotC_Gameplay_Misc_WeaponGunsmith_Base');
		...
		else
		{
			...
			// Iterate through XComWeapon_Gunsmith for every Dynamic Node out there
	...
	{
		//`log("Playing Sound", , 'WotC_Gameplay_Misc_WeaponGunsmith_Base');
		...
		// Tuple overrides sounds
		if (Tuple.Data[0].o != none)
		{
			//`log("Playing Sound from Event", , 'WotC_Gameplay_Misc_WeaponGunsmith_Base');

			Projectiles[Index].SourceAttachActor.PlaySound(SoundCue(Tuple.Data[0].o));
		}
		// Check if XComWeapon has a loaded firing sound
		else if (WeaponVisualizer.FireSound != none)
		{
			//`log("Playing Custom Firing Sound: " @ WeaponVisualizer.FireSound, , 'WotC_Gameplay_Misc_WeaponGunsmith_Base');

			Projectiles[Index].SourceAttachActor.PlaySound(WeaponVisualizer.FireSound);
		}
		else
		{
			//`log("Playing Default Firing Sound", , 'WotC_Gameplay_Misc_WeaponGunsmith_Base');
...
function PlayWeaponAnim()
{
	...
	//`log("" $ GetFuncName() $ "(): Valid Meshes " $ WeaponGSArc.arrMeshes.Length, , 'WotC_Gameplay_Misc_WeaponGunsmith_Base');

	AnimParams.AnimName = (bSuppressorEquipped == true) ? AnimFireSilencedName : 'FF_FireA';
	AnimParams.Looping = false;
	AnimParams.Additive = true;

	// Iterate through XComWeapon_Gunsmith for every Dynamic Node available
	foreach WeaponGSArc.arrMeshes(Mesh, i)
	{
		...
		//Tell our weapon to play its fire animation
		if( WeaponGSArc.arrAdditiveDynamicNodes[i] != None )
		{
			//`log("Mesh " $ i $ " has AnimSequence " $ AnimParams.AnimName $ ". Attempting to play dynamic animation.", , 'WotC_Gameplay_Misc_WeaponGunsmith_Base');

			PlayingSequence = WeaponGSArc.arrAdditiveDynamicNodes[i].PlayDynamicAnim(AnimParams);
			
			PlayingSequences.AddItem(PlayingSequence);
			PlayingSequences.Sort(SortByTimeLeft);

			SetTimer(PlayingSequence.AnimSeq.SequenceLength, false, nameof(BlendOutAdditives), self);
		}
	}

	//`log("Created sequence with " $ PlayingSequences.Length, , 'WotC_Gameplay_Misc_WeaponGunsmith_Base');
}

// AnimSequence may have already started playing so sort the array based on time left.
// Lower value goes forward in the queue
static function int SortByTimeLeft(AnimNodeSequence A, AnimNodeSequence B)
{
...
// Pop earliest additive animation from the list of playing sequences
function BlendOutAdditives()
{
	local CustomAnimParams AnimParams;
	local AnimNodeSequence RemoveSequence;

	// Get the latest animsequence
	RemoveSequence = PlayingSequences[0];
	SourceWeapon.AdditiveDynamicNode.BlendOutDynamicAnim(RemoveSequence, AnimParams.BlendTime);
	
	// Pop earliest additive anim animation
	PlayingSequences.Remove(0, 1);
}